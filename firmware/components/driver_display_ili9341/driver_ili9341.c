	...
	while (w > 0) {
		...
		for (uint16_t currentLine = 0; currentLine < h; currentLine++) {
			for (uint16_t i = 0; i<transactionWidth; i++) {
				internalBuffer[i*2+0] = frameBuffer[x0+i+(y0+currentLine)*ILI9341_WIDTH];
				internalBuffer[i*2+1] = frameBuffer[x0+i+(y0+currentLine)*ILI9341_WIDTH];
			}
